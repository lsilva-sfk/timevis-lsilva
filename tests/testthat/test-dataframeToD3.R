...
test_that("dataframeToD3 works with no input", {
  expect_identical(timevis:::dataframeToD3(), list())
  expect_identical(timevis:::dataframeToD3(data.frame()), list())
})
...
test_that("dataframeToD3 works with a single row", {
  ...
  expect_identical(timevis:::dataframeToD3(df), list,
                   info = info_comp(timevis:::dataframeToD3(df), list))
})
...
test_that("dataframeToD3 errors when given a non-dataframe", {
  expect_error(timevis:::dataframeToD3(50), "input must be a dataframe")
  expect_error(timevis:::dataframeToD3(FALSE), "input must be a dataframe")
})

test_that("dataframeToD3 returns the same whether the dataframe is pure or merged", {
  df <- data.frame(name = c("Dean", "Ben"), age = c(27, 24))
  df_rbind <- rbind(df[1, ], df[2, ])
  expect_identical(timevis:::dataframeToD3(df), timevis:::dataframeToD3(df_rbind),
                   info = info_comp(timevis:::dataframeToD3(df), timevis:::dataframeToD3(df_rbind)))
})
